    ...
    public class MakeDestroy : GameManager
    {
        ...
        private void Update() {
            ...
            if (Input.GetMouseButtonUp(1)) {
				...
				Messenger<GameObject>.Broadcast(GameEvents.BlockUpdate, hit.transform.gameObject, MessengerMode.DONT_REQUIRE_LISTENER);
            }
        ...
        public bool RightClick(Transform arg1, RaycastHit arg2)
        {
			...
			Messenger<GameObject>.Broadcast(GameEvents.BlockUpdate, clone, MessengerMode.DONT_REQUIRE_LISTENER);
            return clone != null;
        }
        ...
        private void PhantomControl(RaycastHit hit)
        {
            if (!hit.collider.CompareTag(Managers.BlockTags[0]) && !hit.collider.CompareTag(Managers.BlockTags[1]) &&
                Vector3.Distance(Managers.Player.transform.position, hit.transform.position) > MaxDistance ||
                Vector3.Distance(Managers.Player.transform.position, hit.transform.position) < MinDistance)
                _highLight.SetActive(false);
